...
원본 거래 180건을 AI에 보내지 않습니다. 기간, 합계, 비율, 상위 카테고리와 월별 변화만 압축해 시스템 프롬프트에 넣습니다. 최근 대화도 최대 6개 메시지만 전송합니다.
...
무료 인스턴스는 쉬고 있던 서버의 첫 요청에 시간이 걸릴 수 있습니다. 프론트는 3초 이상 지연되면 안내 문구를 표시하며, `서버 깨우기` 버튼으로 `/health`를 호출한 뒤 현재 화면의 데이터를 다시 불러올 수 있습니다.
